$(document).ready(function(){
    // 2e6cf20eccde4be4bdd5cebd043d4769
    //https://api.openweathermap.org/data/2.5/weather?q=seoul&appid=2e6cf20eccde4be4bdd5cebd043d4769

    var cityName = []; // 버튼 클릭시 또는 검색바에서 입력 후 엔터를 누를 때, 도시명 데이터를 수집한다.
    console.log(cityName);

    var myAPI = `2e6cf20eccde4be4bdd5cebd043d4769`;
    ...
    function w_info(){
        ...
        //append(), prepend() 자식을 추가하는 메서드는 기존 자식을 남기며 새로운 자식들을 추가한다. 그래서! 신규데이터를 받아 구문을 할 때, 기존 데이터를 삭제(,초기화,비우기)한다.
    ...
    $(document).on("click", ".cities button",function(){
        ...
        var city_txt = $(this).text(); //클릭한 곳의 텍스트 추출
        console.log(city_txt);

        cityName.unshift(city_txt); //cityName이라는 전역 변수에 배열 데이터 첫 번째 인덱스에 추출한 텍스트(도시명)를 넣는다.
        console.log(cityName);

        $(this).remove(); //클릭한 곳 버튼을 제거한다.
    ...
    // 현재 내 위치 정보를 가져와 해당 날씨를 보여준다.

    // http://api.openweathermap.org/data/2.5/weather?lat=35&lon=138&appid={API key}
    ...
    navigator.geolocation.getCurrentPosition(function(position){
        ...
        $.ajax({
            url: `http://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${myAPI}`,
            dataType: "json",
            success: function(data){
                console.log(data);
                ...
                cityName.unshift(city); //브라우저 로딩하자마자 현재 내 위치 배열 데이터에 넣는다.

                w_info(); //함수문을 호출한다 => 배열데이터 개수에 따라 구조 구성하고 그 데이터 내용을 넣겠단 의미
            }
        });
